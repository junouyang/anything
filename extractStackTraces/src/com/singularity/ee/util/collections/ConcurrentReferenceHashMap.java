...
/*
    ...
    public interface StaleEntryListener<V> {
        void removedValue(V valueRemoved);
    }
    ...
    static final class Segment<K, V> extends ReentrantLock implements Serializable {
        ...
        // stale monitor - can be null
        final StaleEntryListener<V> staleEntryListener;

        Segment(int initialCapacity, float lf, ReferenceType keyType,
                ReferenceType valueType, boolean identityComparisons, StaleEntryListener<V> staleEntryListener) {
            loadFactor = lf;
        ...
        final void removeStale() {
            ...
            while ((ref = (KeyReference) refQueue.poll()) != null) {
                V valueRemoved = remove(ref.keyRef(), ref.keyHash(), null, true);
                if (this.staleEntryListener != null && valueRemoved != null) {
                    this.staleEntryListener.removedValue(valueRemoved);
                }
            }
        }
    ...
    public ConcurrentReferenceHashMap(int initialCapacity,
            ...
            ReferenceType keyType, ReferenceType valueType,
            EnumSet<Option> options, StaleEntryListener<V> staleEntryListener) {
        ...
        for (int i = 0; i < this.segments.length; ++i) {
            this.segments[i] = new Segment<K, V>(cap, loadFactor,
                    keyType, valueType, identityComparisons, staleEntryListener);
        }
     ...
     * USE THIS ONE.
     * <p/>
     * Method to get a map with
     * 1) Default load factor, Default Initital Capacity, Default Concurrency Level.
     * 1) Weak Keys
     * 2) Strong values
     * 3) Identity Comparison is used for keys. If you put a value in with Obj1,
     * only if obj1==key in get(key)  will you be able to retrieve the value.-obj1.equals(key) will not work.
     * @param option the key type option.
     */
    public ConcurrentReferenceHashMap(ConcurrentReferenceHashMap.Option option) {
        this(DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL,
                DEFAULT_KEY_TYPE, DEFAULT_VALUE_TYPE, EnumSet.of(option), null);
    }
     ...
     * Resizing may be performed when the average number of elements per
     * bin exceeds this threshold.
     * @exception IllegalArgumentException if the initial capacity of
     ...
     * specified value
     * @exception NullPointerException if the specified value is null
     */
    ...
    /**
     * {@inheritDoc}
     * @exception NullPointerException if any of the arguments are null
     */
